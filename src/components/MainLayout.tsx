...
import { SystemPowerActionsProvider } from '../contexts/SystemPowerActionsContext';
...
const MainLayout: React.FC = () => {
  ...
  const isPowerActionDisabled =
    isPowerActionPending || Boolean(countdownAction) || isConfirmationOpen;

  const handlePowerActionRequest = useCallback(
    (action: PowerAction) => {
  ...
  const powerActionsContextValue = useMemo(
    () => ({
      requestPowerAction: handlePowerActionRequest,
      isPowerActionDisabled,
    }),
    [handlePowerActionRequest, isPowerActionDisabled]
  );

  const handleUserPowerAction = (action: PowerAction) => {
    handleUserMenuClose();
  ...
  return (
    <SystemPowerActionsProvider value={powerActionsContextValue}>
      <Box
        sx={{
          display: 'flex',
          minHeight: '100svh',
            ...
            >
              ...
              >
                ...
                <MenuItem
                  ...
                  onClick={() => handleUserPowerAction('reboot')}
                  sx={{
                ...
                <MenuItem
                  disabled={isPowerActionDisabled}
                  onClick={() => handleUserPowerAction('poweroff')}
                  sx={{
