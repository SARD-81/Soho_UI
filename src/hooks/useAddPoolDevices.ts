...
export const useAddPoolDevices = (options: UseAddPoolDevicesOptions = {}) => {
  ...
  const {
    data: fetchedVdevType,
    isFetching: isVdevLoading,
    error: vdevQueryError,
  ...
  useEffect(() => {
    if (fetchedVdevType === undefined) {
      return;
    }

    const normalized = normalizeVdevType(fetchedVdevType);

    setVdevType(fetchedVdevType);
    setVdevError(normalized ? null : 'نوع آرایه این فضا قابل شناسایی نیست.');
  }, [fetchedVdevType]);
  ...
  useEffect(() => {
    ...
    setVdevError(vdevQueryError.message);
    setVdevType('');
  }, [vdevQueryError]);
  ...
  const effectiveVdevType = useMemo<string>(
